...
export const SERVERS: Server[] = [
  { id: "filiflix", name: "FiliFlix Server (Default)", url: (type: string, id: string, s?: number, e?: number) => type === 'tv' ? `https://autoembed.co/tv/tmdb/${id}-${s || 1}-${e || 1}` : `https://autoembed.co/movie/tmdb/${id}` },
  { id: "vidlink", name: "FiliFilipino Server", url: (type: string, id: string, s?: number, e?: number) => `https://vidlink.pro/${type}/${id}${type === 'tv' ? `/${s || 1}/${e || 1}` : ''}?autoplay=1` },
